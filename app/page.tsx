...
export default function Home() {
  ...
  {['BUSINESS DATA', 'INGESTION', 'TRANSFORMATION', 'GOVERNANCE', 'ANALYTICS / AI', 'BUSINESS VALUE'].map((label, index) => (
    ...
    <section id="sobre-mi" className="bg-[#d7fb5f] px-5 py-24 text-[#081014] lg:px-8 lg:py-28"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.7fr_1.3fr] lg:items-end"><div><p className="mb-5 font-mono text-xs tracking-[.25em]">08 / SOBRE MÍ</p><div className="group relative min-h-64 overflow-hidden border border-[#081014]/30 bg-[#c2e64f] p-4"><div className="absolute inset-4 border border-[#081014]/20" /><div className="relative flex h-full min-h-56 flex-col items-center justify-center"><div className="flex h-24 w-24 items-center justify-center rounded-full border border-[#081014]/40 font-display text-3xl transition-transform duration-500 group-hover:scale-105">VM</div><p className="mt-5 font-mono text-[10px] tracking-[.18em]">VICTOR MIRANDA</p><p className="mt-2 text-xs text-[#304044]">DATA OWNER · DATA ENGINEER · BI & DATA</p></div></div></div><div><h2 className="mb-5 font-display text-3xl leading-tight tracking-[-.05em] md:text-5xl">Datos, tecnología y negocio.</h2><p className="max-w-4xl font-display text-2xl leading-tight tracking-[-.04em] md:text-4xl">Soy Victor Miranda, Data Owner & Data Engineer, especializado en convertir necesidades de negocio en soluciones de datos confiables, gobernadas y escalables.</p><p className="mt-7 max-w-2xl text-base leading-7 text-[#304044]">Mi trabajo combina ingeniería de datos, arquitectura cloud, gobierno, analítica y automatización para construir soluciones que puedan operar y crecer en entornos empresariales.</p><div className="mt-10 border-t border-[#081014]/25 pt-5"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#304044]">Conecta conmigo</p><div className="mt-4 flex flex-wrap gap-3"><a aria-label="LinkedIn" href={contactLinks.linkedin} className="flex items-center gap-2 border border-[#081014]/30 px-3 py-2.5 text-xs transition-colors hover:bg-[#081014] hover:text-[#d7fb5f]"><Linkedin size={16} /> LinkedIn</a><a aria-label="GitHub" href={contactLinks.github} className="flex items-center gap-2 border border-[#081014]/30 px-3 py-2.5 text-xs transition-colors hover:bg-[#081014] hover:text-[#d7fb5f]"><Github size={16} /> GitHub</a><a aria-label="WhatsApp" href="#contacto" className="flex items-center gap-2 border border-[#081014]/30 px-3 py-2.5 text-xs transition-colors hover:bg-[#081014] hover:text-[#d7fb5f]"><MessageCircle size={16} /> WhatsApp</a><a aria-label="Email" href={`mailto:${contactLinks.email}`} className="flex items-center gap-2 border border-[#081014]/30 px-3 py-2.5 text-xs transition-colors hover:bg-[#081014] hover:text-[#d7fb5f]"><Mail size={16} /> Email</a></div></div></div></div></section>

    <section className="border-b border-white/10 px-5 py-20 lg:px-8"><div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-2 lg:grid-cols-5">{['De datos a decisiones.', 'De procesos manuales a automatización.', 'De consultas costosas a arquitecturas optimizadas.', 'De complejidad técnica a soluciones de negocio.', 'De datos dispersos a información gobernada.'].map((line, index) => <motion.p key={line} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * .08 }} viewport={{ once: true }} className="border-l border-[#d7fb5f] pl-4 font-display text-lg leading-tight text-[#c1cbca]">{line}</motion.p>)}</div></section>
